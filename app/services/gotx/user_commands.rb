  ...
  module UserCommands
    ...
    application_command(:membership) do |event|
      ...
      event.respond(content: 'Update premium membership status:') do |_, view|
        view.row do |r|
          ...
          r.button(label: 'Add membership', style: :success, custom_id: "#{user.id}_update_premium_member_add", emoji: EMOJI)
        end
      end
    end
    ...
    button(custom_id: /\d_update_premium_member_/) do |event|
      ...
      event.update_message(content: 'Update membership level:') do |_, view|
        view.row do |row|
          row.button(label: 'Supporter', style: :danger, custom_id: "#{user.id}_add_premium_membership_supporter", emoji: EMOJI)
          row.button(label: 'Champion', style: :success, custom_id: "#{user.id}_add_premium_membership_champion", emoji: EMOJI)
          row.button(label: 'Legend', style: :primary, custom_id: "#{user.id}_add_premium_membership_legend", emoji: EMOJI)
        end
      end
    end
